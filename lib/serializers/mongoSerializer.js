...
function convertNotOperator(options, args)
{
  ...
  for (var i = 0, l = args.length; i < l; ++i)
  {
    ...
    for (var property in nested)
    {
      if (!nested.hasOwnProperty(property) || property[0] === '$')
      {
        continue;
      }
      ...
      for (var nestedProperty in nestedValue)
      {
        if (nestedValue.hasOwnProperty(nestedProperty)
          && nestedProperty[0] === '$')
        {
